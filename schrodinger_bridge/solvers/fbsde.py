        ...
        for stage in range(cfg.num_stages):
            ...
            for step in range(cfg.steps_per_stage):
                ...
                (loss, metrics), grads = jax.value_and_grad(
                    ...
                    params['backward'],         # trainable
                    self._backward_factory,
                    params['forward'],           # frozen (stop_gradient inside)
                    self._forward_factory,
                    x_batch, t_batch, k_loss,
                )

                params['backward'], bwd_opt = adam_update(
                    bwd_opt, grads, params['backward'], lr=cfg.learning_rate
                )
            ...
            for step in range(cfg.steps_per_stage):
                ...
                (loss, metrics), grads = jax.value_and_grad(
                    ...
                    params['forward'],           # trainable
                    self._forward_factory,
                    params['backward'],           # frozen (stop_gradient inside)
                    self._backward_factory,
                    x_batch, t_batch, k_loss,
                )

                params['forward'], fwd_opt = adam_update(
                    fwd_opt, grads, params['forward'], lr=cfg.learning_rate
                )
