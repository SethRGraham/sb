...
class SBSolver(abc.ABC):
    ...
    def train(
        ...
        final_checkpoint_path = self._maybe_save_checkpoint(
            ...
            opt_state=opt_state,
            loss_history=loss_history,
            metrics={'loss': loss_history[-1] if loss_history else float('nan')},
            final=True,
            metadata=metadata,
        )
    ...
    def _checkpoint_filename(self, config: TrainingConfig, step: int, final: bool) -> str:
        """Build a deterministic checkpoint filename for this solver."""
        solver_name = self.solver_type.name.lower()
        if final:
            suffix = "final"
        else:
            suffix = f"step_{step:08d}"
        return f"{config.checkpoint_prefix}_{solver_name}_{suffix}.pkl"

    def _checkpoint_path(self, config: TrainingConfig, step: int, final: bool) -> Path:
        if config.checkpoint_dir is None:
            raise ValueError("checkpoint_dir must be set to build a checkpoint path")
        return Path(config.checkpoint_dir) / self._checkpoint_filename(config, step, final)
    ...
    def _maybe_save_checkpoint(
        ...
        *,
        ...
        """Save a checkpoint when the training config asks for one."""
        if config.checkpoint_dir is None:
            return None
        if final:
            if not config.save_final_checkpoint:
                return None
        elif config.checkpoint_every <= 0 or step % config.checkpoint_every != 0:
            return None

        path = self._checkpoint_path(config, step, final)
    ...
    def _checkpoint_state(self) -> Dict[str, Any]:
        """Return subclass-specific state to include in checkpoints."""
        return {}

    def _restore_checkpoint_state(self, state: Dict[str, Any]) -> None:
        """Restore subclass-specific checkpoint state."""
        del state

    def _host_value(self, value: Any) -> Any:
        """Move arrays in a nested object to host memory before pickling."""
        if is_dataclass(value) and not isinstance(value, type):
            data = {
                field.name: self._host_value(getattr(value, field.name))
                for field in fields(value)
            }
            return type(value)(**data)
        if isinstance(value, dict):
            return {key: self._host_value(item) for key, item in value.items()}
        if isinstance(value, list):
            return [self._host_value(item) for item in value]
        if isinstance(value, tuple):
            values = [self._host_value(item) for item in value]
    ...
    def save_checkpoint(
        ...
        *,
        ...
        """Serialize solver parameters and training state to disk.

        Args:
            path: Destination checkpoint file.
            params: Parameters to save. Defaults to the trained parameters.
            opt_state: Optional optimizer state.
            step: Training step associated with the checkpoint.
            loss_history: Losses observed so far.
            metrics: Latest metrics.
            training_config: Training configuration used for the run.
        ...
        if params is None:
            if self._params is None:
                raise ValueError("No parameters available to checkpoint.")
            params = self._params
        ...
        payload = {
            ...
            'solver_state': self._checkpoint_state(),
        }
        payload = self._host_value(payload)
    ...
    def load_checkpoint(self, path: Union[str, Path]) -> Dict[str, Any]:
        """Load a checkpoint into this solver instance.

        The solver must be constructed with the same problem/configuration that
        produced the checkpoint. For neural solvers this includes the same
        network factory or architecture config. The method restores parameters
        and any subclass-specific runtime state needed for inference.

        Args:
            path: Checkpoint file to load.

        Returns:
            The deserialized checkpoint payload.
        """
        checkpoint_path = Path(path)
        with checkpoint_path.open("rb") as f:
            payload = pickle.load(f)

        expected_class = payload.get('solver_class')
        if expected_class is not None and expected_class != type(self).__name__:
            raise ValueError(
                f"Checkpoint was saved by {expected_class}, "
                f"but this solver is {type(self).__name__}."
            )

        self._params = payload['params']
        self._is_trained = True
        self._restore_checkpoint_state(payload.get('solver_state', {}))
        return payload
