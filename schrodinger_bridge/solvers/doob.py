...
class DoobHTransformSolver(SBSolver):
    ...
    def _restore_checkpoint_state(self, state: Dict[str, Any]) -> None:
        ...
        self._A = state.get('A')
        self._source_samples = state.get('source_samples')
        self._target_samples = state.get('target_samples')
        self._bandwidth = state.get('bandwidth')
        if self._params is not None:
            if self._A is None:
                self._A = self._params.get('A')
            if self._source_samples is None:
                self._source_samples = self._params.get('source_samples')
            if self._target_samples is None:
                self._target_samples = self._params.get('target_samples')
    ...
    def _analytical_log_h_coefficients(self, t: Scalar) -> Tuple[Array, Array]:
        """Return affine coefficients for ∇log h(x, t) = G_t x + g_t.

        For the analytical Gaussian solver, the bridge correction is affine in x,
        so log h is a quadratic function up to an additive constant.
        """
        t_scalar = jnp.asarray(t).reshape(())
        m0 = self._params['m0']
        m1 = self._params['m1']
    ...
    def compute_log_h_gradient(self, x: Array, t: Scalar) -> Array:
        """Compute ∇log h(x, t) implied by the Doob transform.

        For the Ito convention used in this library,
            b*(x, t) = b_ref(x, t) + sigma(t)^2 ∇log h(x, t).

        This method exposes the h-transform gradient directly so downstream
        experiments can study the potential field rather than only the drift.
        """
        if not self._is_trained:
            raise RuntimeError("Solver must be trained before computing the h-transform")
    ...
    def compute_log_h(
        ...
        quadrature_steps: int = 17,
    ) -> Array:
        """Estimate log h(x, t) up to an additive constant.

        We anchor log h(anchor, t) = 0 and integrate the estimated gradient
        along the straight-line path from the anchor to x. For analytical
        Gaussian cases this recovers the quadratic potential exactly up to
        numerical quadrature; for kernel methods it gives a stable surrogate
        scalar potential whose gradient matches the Doob correction field.
        """
        if not self._is_trained:
            raise RuntimeError("Solver must be trained before computing the h-transform")
        ...
        dim = x.shape[-1]
        if anchor is None:
            anchor = jnp.zeros((dim,))
        anchor = jnp.asarray(anchor).reshape(1, dim)

        if self._method == 'analytical':
            linear, bias = self._analytical_log_h_coefficients(t)

            def quadratic_eval(points: Array) -> Array:
                quad = 0.5 * jnp.einsum("bi,ij,bj->b", points, linear, points)
                affine = points @ bias
                return quad + affine

            anchor_val = quadratic_eval(anchor)[0]
            return quadratic_eval(x) - anchor_val

        num_steps = max(int(quadrature_steps), 2)
        alphas = jnp.linspace(0.0, 1.0, num_steps)

        def single_log_h(xi: Array) -> Scalar:
            delta = xi - anchor[0]
            path = anchor + alphas[:, None] * delta[None, :]
            grad = self.compute_log_h_gradient(path, t)
            integrand = jnp.einsum("sd,d->s", grad, delta)
            return jnp.trapezoid(integrand, alphas)

        return jax.vmap(single_log_h)(x)
    ...
    def compute_h(
        ...
        """Compute a positive h-transform field normalized by h(anchor, t)=1."""
        ...
        return jnp.exp(jnp.clip(log_h, a_min=-60.0, a_max=60.0))
    ...
    def extract_potential(
        ...
        """Extract the Doob h-transform as a potential representation.

        The returned ``PotentialRepresentation`` uses:
        - ``potential_fn(x, t) = h(x, t)``
        - ``potential_grad_fn(x, t) = ∇log h(x, t)``

        This matches the base ``PotentialRepresentation.to_drift()``
        convention, which reconstructs the bridge drift via

            b*(x, t) = b_ref(x, t) + sigma(t)^2 ∇log h(x, t).

        Note that for kernel-based Doob solvers, ``h(x, t)`` is a derived
        quantity obtained from ``compute_log_h()`` and anchor-normalized so
        that ``h(anchor, t) = 1``.
        """
        ...
        def potential_fn(x: Array, t: Scalar) -> Array:
        ...
        def potential_grad_fn(x: Array, t: Scalar) -> Array:
            return self.compute_log_h_gradient(x, t)

        return PotentialRepresentation(potential_fn, potential_grad_fn)
