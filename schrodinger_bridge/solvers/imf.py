...
class IMFSolver(SBSolver):
    ...
    def _train_step_jit(
        ...
        if self.imf_config.use_ot_coupling:
            coupling = self._compute_ot_coupling(
                x0, x1, self.imf_config.ot_regularization
            )
            x1 = x1[coupling]

        if phase == 'forward':
            if use_refined_targets:
    ...
    def train_step(
        ...
        use_refined_targets = bool(use_imf and self._imf_iteration > 0)
    ...
    def train(
        ...
        for step in range(self.imf_config.steps_per_iteration):
            ...
            params, opt_state, metrics = self.train_step(
                step_key, params, opt_state, config.batch_size, phase=phase, use_imf=False
            )
            ...
            checkpoint_path = self._maybe_save_checkpoint(
                ...
                metrics=metrics,
                metadata={'imf_iteration': self._imf_iteration, 'phase': phase},
            )
        ...
        for imf_iter in range(self.imf_config.num_imf_iterations):
            ...
            for step in range(self.imf_config.steps_per_iteration // 2):
                k2, step_key = jax.random.split(k2)
                params, opt_state, metrics = self.train_step(
                    step_key, params, opt_state, config.batch_size, phase='forward', use_imf=True
                )
                all_losses.append(metrics['loss'])
                global_step += 1
                ...
                checkpoint_path = self._maybe_save_checkpoint(
                    ...
                    loss_history=all_losses,
                    metrics=metrics,
                    metadata={'imf_iteration': self._imf_iteration, 'phase': 'forward'},
                )
                if checkpoint_path is not None:
                    checkpoint_paths.append(checkpoint_path)
            ...
            for step in range(self.imf_config.steps_per_iteration // 2):
                k2, step_key = jax.random.split(k2)
                params, opt_state, metrics = self.train_step(
                    step_key, params, opt_state, config.batch_size, phase='backward', use_imf=True
                )
                all_losses.append(metrics['loss'])
                global_step += 1
                ...
                checkpoint_path = self._maybe_save_checkpoint(
                    ...
                    loss_history=all_losses,
                    metrics=metrics,
                    metadata={'imf_iteration': self._imf_iteration, 'phase': 'backward'},
                )
                if checkpoint_path is not None:
                    checkpoint_paths.append(checkpoint_path)
    ...
    def _checkpoint_state(self) -> Dict[str, Any]:
        return {'imf_iteration': self._imf_iteration}

    def _restore_checkpoint_state(self, state: Dict[str, Any]) -> None:
        self._imf_iteration = state.get('imf_iteration', 0)
