...
Core API
--------
Neural SB solvers learn time-conditioned vector or scalar fields. The network
factory abstracts how those fields are parameterized. The solver only ever
...
Solver-specific output semantics (what f MEANS to each solver):
    ...
    MalliavinSolver   | grad log phi_t(x)         | D
    IPFDSBSolver      | transition mean map       | D
