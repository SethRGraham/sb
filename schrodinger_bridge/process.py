...
def _covariance_to_diffusion_coefficient(
    covariance: Union[Scalar, Array],
    x: Array,
) -> Union[Scalar, Array]:
    """Convert square covariance outputs to diffusion coefficients."""
    covariance = jnp.asarray(covariance)
    x = jnp.atleast_2d(x)
    dim = x.shape[-1]

    if covariance.ndim == 2 and covariance.shape == (dim, dim):
        return jnp.linalg.cholesky(
            covariance + 1e-8 * jnp.eye(dim, dtype=covariance.dtype)
        )

    if covariance.ndim == 3 and covariance.shape[-2:] == (dim, dim):
        eye = jnp.eye(dim, dtype=covariance.dtype)
        return jax.vmap(lambda a: jnp.linalg.cholesky(a + 1e-8 * eye))(covariance)
...
class BridgeProcess:
    ...
    def diffusion(self, x: Array, t: Scalar) -> Union[Scalar, Array]:
        ...
        sigma = self.problem.sigma(x, t)
        if self.metadata.get("diffusion_matrix_is_covariance", False):
            return _covariance_to_diffusion_coefficient(sigma, x)
        return sigma
