...
class Heun(Integrator):
    ...
    def step(
        ...
        # Predictor (Euler)
        b1 = drift(x, t)
        x_pred = x + b1 * dt + sigma * dW
        
        # Corrector
        b2 = drift(x_pred, t + dt)
        sigma2 = diffusion(t + dt)
        
        x_new = x + 0.5 * (b1 + b2) * dt + 0.5 * (sigma + sigma2) * dW
...
class AdaptiveIntegrator(Integrator):
    ...
    def step(
        ...
        # Euler step (order 1)
        b1 = drift(x, t)
        x_euler = x + b1 * dt + sigma * dW
        
        # Heun step (order 2)
        b2 = drift(x_euler, t + dt)
        x_heun = x + 0.5 * (b1 + b2) * dt + sigma * dW
...
class SpectralIntegrator(Integrator):
    ...
    def step(
        ...
        x_new = x_new + sigma * jnp.sqrt(jnp.abs(dt)) * noise
