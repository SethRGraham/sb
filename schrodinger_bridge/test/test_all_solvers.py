...
# Problems (available to both script and pytest)
problem = SBProblem(
...
def run_solver(name, solver_cls, prob, key, train_config=None, **kwargs):
    print(f"\n{'─' * 50}")
...
def main():
    # use module-level PRNGKey as starting key
    key = KEY
    # Test solvers
    key, subkey = jax.random.split(key)
    results['score'] = run_solver("Score-Based", ScoreBasedSolver, problem, subkey,
        config=ScoreBasedConfig(hidden_dims=(64, 64), learning_rate=1e-3))

    key, subkey = jax.random.split(key)
    results['fbsde'] = run_solver("FBSDE", FBSDESolver, problem, subkey,
        fbsde_config=FBSDEConfig(hidden_dims=(64, 64), learning_rate=1e-3, method='soc'))

    key, subkey = jax.random.split(key)
    results['doob_analytical'] = run_solver("Doob-Analytical", DoobHTransformSolver, problem_g2g, subkey,
        config=DoobConfig(method='analytical'))

    key, subkey = jax.random.split(key)
    results['doob_kernel'] = run_solver("Doob-Kernel", DoobHTransformSolver, problem, subkey,
        config=DoobConfig(method='kernel', num_inducing_points=150))

    key, subkey = jax.random.split(key)
    results['rkhs'] = run_solver("RKHS", RKHSSolver, problem, subkey,
        rkhs_config=RKHSConfig(num_inducing=150, num_time_points=10))

    key, subkey = jax.random.split(key)
    results['imf'] = run_solver("IMF", IMFSolver, problem, subkey,
...
def test_score_smoke():
    ...
    train_cfg = TrainingConfig(num_iterations=1, batch_size=8)
    res = run_solver("Score-Based-Smoke", ScoreBasedSolver, problem, key,
                     train_config=train_cfg,
                     config=ScoreBasedConfig(hidden_dims=(16, 16), learning_rate=1e-3))
    assert 'success' in res


def test_fbsde_smoke():
    key = jax.random.PRNGKey(1)
    train_cfg = TrainingConfig(num_iterations=1, batch_size=8)
    res = run_solver("FBSDE-Smoke", FBSDESolver, problem, key,
                     train_config=train_cfg,
                     fbsde_config=FBSDEConfig(hidden_dims=(16, 16), learning_rate=1e-3, method='soc'))
    assert 'success' in res
